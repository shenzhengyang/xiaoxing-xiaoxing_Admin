...
function updateToServerSide(item, data){
    var itemid = item;
    var update_data = {'id':itemid,'eid': data[0], 'lat': data[1], 'lng': data[2],'radius':data[3]}; //傳送至伺服器端的Data產生處，需手動修改對應表格
    updatePost( '../Table/railUpdate', update_data);
}
$(function () {
    //初始化变量
    var $table = $('#table');
    ...
    function remove(){
        ...
        if(ids==""){
            ...
            removePost('../Table/railRemove',{"id":ids});
        }
    }
    //添加数据函数
    function add(){
        var data="{";
        var data_name=new Array('eid','lat','lng','radius');
        ...
        addPost('../Table/railAdd',data_json);
    }
    //删除按钮点击事件
