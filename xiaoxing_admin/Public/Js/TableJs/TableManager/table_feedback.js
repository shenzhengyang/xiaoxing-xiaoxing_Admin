...
function updateToServerSide(item, data){
    var itemid = item;
    var update_data = {'id':itemid,'uid': data[0], 'time': data[1], 'desc': data[2], 'hasRepaired': data[3]}; //傳送至伺服器端的Data產生處，需手動修改對應表格
    updatePost( '../Table/feedbackUpdate', update_data);
}
$(function () {
    //初始化变量
    var $table = $('#table');
    ...
    function remove(){
        ...
        if(ids==""){
            ...
            removePost('../Table/feedbackRemove',{"id":ids});
        }
    }
    //添加数据函数
    function add(){
        var data="{";
        var data_name=new Array('uid','time','desc','hasRepaired');
        ...
        addPost('../Table/feedbackAdd',data_json);
    }
    //删除按钮点击事件
