...
function updateToServerSide(item, data){
    var itemid = item;
    var update_data = {'imei': data[0], 'phone': data[1]}; //傳送至伺服器端的Data產生處，需手動修改對應表格
    updatePost( '../Table/equipUpdate', update_data);
}
$(function () {
    //初始化变量
    var $table = $('#table');
    ...
    //删除函数
    function remove(){
        var imei = $.map($table.bootstrapTable('getSelections'), function (row) {
            return row.imei;
        });
        if(imei==""){
            alert("未选择要删除的数据！");
        }else{
            removePost('../Table/equipRemove',{"imei":imei});
        }
    }
    //添加数据函数
    function add(){
        var data="{";
        var data_name=new Array('imei','phone');
        ...
        addPost('../Table/equipAdd',data_json);
    }
    //删除按钮点击事件
