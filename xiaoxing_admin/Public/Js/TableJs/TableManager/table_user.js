...
function updateToServerSide(item, data){
    var itemid = item;
    var update_data = {'id':itemid,'username': data[0], 'password': data[1]}; //傳送至伺服器端的Data產生處，需手動修改對應表格
    updatePost( '../Table/userUpdate', update_data);
}
$(function () {
    //初始化变量
    var $table = $('#table');
    ...
    function remove(){
        ...
        if(ids==""){
            ...
            removePost('../Table/userRemove',{"id":ids});
        }
    }
    //添加数据函数
    function add(){
        var data="{";
        var data_name=new Array('username','password');
        ...
        addPost('../Table/userAdd',data_json);
    }
    //删除按钮点击事件
